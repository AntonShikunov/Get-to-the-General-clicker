    ...
    private void CheckOffline()
    {
        ...
        if (PlayerPrefs.HasKey("LastSession"))
        {
            ...
            if (lvlOffMoney == 1) // 1 lvl
            {
                if (ts.Hours >= 1)
                {
                    int fromFiveSec = (((((Game.wage + Game.bonusMoney) + Authority.WageAuthority) * Game.WageForCadet) * 5) / 100);
                    EarnMoneyOff = (fromFiveSec * 720);
                    Game.money += (fromFiveSec * 720);
                    Status.totalEarnedMoney += (fromFiveSec * 720);
                    TimeOfflineText.text = string.Format("Вы отсутствовали в игре максимальное время: 1 час. \nВаш заработок: {0} денег", (fromFiveSec * 720));
                }
            ...
            if (lvlOffMoney == 3) // 3 lvl
            {
                ...
                if (lvlOffMoney == 4) // 4 lvl
                {
                    if (ts.Hours >= 12)
                    {
                        int fromFiveSec = (((((Game.wage + Game.bonusMoney) + Authority.WageAuthority) * Game.WageForCadet) * 15) / 100);
